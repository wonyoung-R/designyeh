...
export function ServicePage({ service }: { service: ServiceContent }) {
  ...
  return <>
    ...
    <header className="docent contact-nav"><Link className="wordmark" href="/">designyeh<span className="wm-period">.</span></Link><Link className="contact-home" href="/contact/">프로젝트 상담</Link></header>
    <main className="service-detail">
      <section className="room room-entry" aria-labelledby="service-title">
        ...
        <div className="service-sheet">
          ...
          <div className="cta-row"><Link className="cta cta-primary" href="/contact/">프로젝트 상담하기 →</Link><a className="cta cta-secondary" href="#service-works">관련 작품 보기 ↓</a></div>
      ...
      <section className="room room-final" aria-labelledby="next-title"><div className="service-sheet"><h2 id="next-title">필요한 범위부터 이야기해 주세요.</h2><p>현재 상황, 목표, 준비된 자료와 필요한 기능 또는 반복 업무를 알려주시면 포함·제외 범위와 승인·인계 기준을 함께 확인합니다.</p><div className="cta-row"><Link className="cta cta-primary" href="/contact/">프로젝트 상담하기 →</Link></div><nav className="service-related" aria-label="함께 살펴볼 서비스">{services.filter(item => item.slug !== service.slug).map(item => <Link className="service-link" href={`/${item.slug}/`} key={item.slug}>{item.name} 살펴보기 →</Link>)}</nav></div><div className="baseboard" /></section>
